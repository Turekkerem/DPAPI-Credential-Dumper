...
namespace GeckoAndIECookies {
    ...
    inline void ReadFirefoxLikeCookies(const std::wstring& profileDir,
        ...
        if (sqlite3_open(dbPathStr.c_str(), &db) != SQLITE_OK) {
            std::cerr << "[" << browserName << "] cookies.sqlite not found in profile.\n";
            return;
        }

        const char* sql = "SELECT host, name, value FROM moz_cookies;";
    ...
    inline void ReadIECookies() {
        ...
        if (hFind == INVALID_HANDLE_VALUE) {
            std::cout << "\n=== Internet Explorer ===\nNo cookie files found (or directory does not exist).\n";
            return;
        }
        std::cout << "\n=== Internet Explorer (plaintext) ===\n";
        do {
            if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                ...
                if (file.is_open()) {
                    ...
                    std::cout << "File: " << findData.cFileName
                            << " | IE cookie read\n";
                    file.close();
                }
            }
